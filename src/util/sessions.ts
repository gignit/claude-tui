/**
 * Session TRANSCRIPT parsing for the current project — history replay
 * (resume/fork), rewind points, and todo reconstruction.
 *
 ...
 *
 * Session LISTING and titles use the SDK's standalone session-store
 * functions instead (see listSessionSummaries in src/agent/client.ts).
 * Transcript parsing stays hand-rolled because we need branch-aware
 * DisplayItems (active-branch filtering for rewind) that the SDK's
 * getSessionMessages doesn't provide.
 */
