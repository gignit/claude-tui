...
| Launcher dies with `preload not found` after an opentui upgrade | opentui ≥0.4 publishes compiled `scripts/preload.js`; the 0.1.x `scripts/preload.ts` no longer exists in the package | Makefile launcher points at `preload.js`; the `"@opentui/solid/preload"` export in bunfig.toml still resolves for dev |
| Blockquote left-bars / horizontal rules invisible on opentui ≥0.4 | `MarkdownRenderable` colors that chrome from the syntaxStyle `conceal` scope's fg. Our 0.1.x theme set conceal fg = background to hide `**` markers — but 0.4 *removes* markers (marked-token rendering + conceal source maps) instead of painting them | Map `conceal` to a visible dim color (`md.rule`) in `theme.tsx`; never background-colored. Blockquote body styles via `markup.quote`. |
| Need opentui internals but `node_modules/@opentui/*` is dist-only now | ≥0.4 packages ship compiled JS + `.d.ts`, no `src/` | Real source: coder module tools (`module: "@opentui/core"`) or the checkout under `~/src/modules/opentui/` |
